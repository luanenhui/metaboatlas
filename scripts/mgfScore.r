...
len = min(length(start), 50)  # maximum spectra to compare
...
result = sapply(1:(length(info)-1), function(i){
# result = sapply(1, function(i){
  spec.exp = info[[i]]$ms2
  
  sapply((i+1):length(info), function(j){
    spec.lib = info[[j]]$ms2
    score = GetMatchResult(spec.exp = spec.exp, spec.lib = spec.lib, ppm=tolerance*1000)
    if (score > cutoff){
      result = sprintf("%s&%s&%s&%s&%s&%s&%s", i, j, score, info[[i]]$mz, info[[i]]$rt, info[[j]]$mz, info[[j]]$rt)
    } else {
      result = NA
    }
    result
...
if (result==""){result = "-&-&-&no any two spectrum have a score > cutoff&-&-&-"}
